...
def load_upload_service():
    upload_service = os.getenv('UPLOAD_SERVICE', 'Hdfs')

    try:
        module = importlib.import_module(f'app.upload_services.{upload_service}UploadService')
        upload_service_class = getattr(module, upload_service)
        return upload_service_class()
    except (ImportError, AttributeError) as e:
        print(e)
        raise ValueError(f"Unknown authentication service class: {upload_service}")